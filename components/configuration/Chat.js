...
function Chat({ params, userMessage, isOrchestralModel = false, searchParams, isEmbedUser, draftPrompt }) {
  ...
  const channelIdentifier = useMemo(() => {
    ...
    if (isPublished) {
      ...
      return (params.org_id + "_" + params?.id + "_" + publishedVersionId + "_" + userId).replace(/ /g, "_");
    } else {
      // For draft versions, include the version
      return (params.org_id + "_" + params?.id + "_" + searchParams?.version + "_" + userId).replace(/ /g, "_");
    }
  }, [params, searchParams, publishedVersionId, userId]);
