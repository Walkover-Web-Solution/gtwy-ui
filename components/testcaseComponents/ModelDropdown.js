...
const TestCaseModelDropdown = ({ selectedModels = [], onChange, selectedVersions = [], versions = [], bridgeId }) => {
  ...
  const getVersionsMissingService = (service) => {
    ...
    selectedVersions.forEach((version) => {
      ...
      if (!versionId) return;
      const servicesForVersion = apikeysByVersion[versionId] || [];
      if (servicesForVersion.includes(service)) return;
                ...
                <div key={group.provider}>
                  <div className="flex items-center justify-between gap-2 px-2.5 pt-1.5 pb-1">
                    ...
                    {isServiceUnavailable && (
                      <span
                        className="inline-flex items-center gap-1 text-[10px] font-bold text-error bg-error/10 px-1.5 py-0.5 rounded"
                        title={unavailableTooltip}
                        data-testid={`testcase-model-service-missing-${group.provider}`}
                      >
                        <AlertCircle size={10} />
                        No Api Key: {missingLabels}
                      </span>
                    )}
                  </div>
                  {group.models.map((model) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={disabled}
                        title={disabled ? unavailableTooltip : ""}
                        className={`w-full flex items-center justify-between gap-2 px-2.5 py-2 rounded-[9px] text-left text-[13.5px] transition-colors ${
                          disabled
