...
function Canvas({
  ...
  const previousTextareaHeight = useRef(null);
  ...
  // Scroll adjustment when textarea is resized (e.g., via drag handle)
  useEffect(() => {
    ...
    previousTextareaHeight.current = textarea.offsetHeight;

    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        const newHeight = entry.contentRect.height;
        const prevHeight = previousTextareaHeight.current;

        if (prevHeight !== null && newHeight !== prevHeight) {
          // When textarea grows, scroll the messages container down by the difference
          const heightDelta = newHeight - prevHeight;
          messagesContainer.scrollTop += heightDelta;
        }

        previousTextareaHeight.current = newHeight;
      }
    });
