...
function Page({ params, searchParams }) {
  ...
  useEffect(() => {
    const fetchInitialData = async (resolvedParams, resolvedSearchParams) => {
      setLoading(true);
      dispatch(clearThreadData());
      // historyEmbed mode with a specific message_id: fetch just that message
      const rawEmbedMessageId = search.get("message_id");
      // Treat "None"/"null"/"undefined" (produced by some upstream callers)
      // as absent so we don't call the API with a bogus id like /message_id/None.
